...
const DraftRow = ({ draftEvent , handleonDeleteClick }: DraftRowProps) => {
    ...
    const handleOnEditClick = () =>{
        navigate(`/organizer/EditEvent/${draftEvent.id}`)
    }
    return (
        <tr className="hover:bg-gray-50 transition-colors">
            ...
            <td className="p-3 border-b border-gray-200 text-gray-600">{draftEvent.venueName}</td>
            <td className="p-3 border-b border-gray-200">
                ...
                <button className="p-3 py-1.5 text-red-600 rounded-md text-sm  hover:bg-orange-100 transition-colors">
                    <Trash2 onClick={()=>handleonDeleteClick(draftEvent.id ?? 0)} />
                </button>
            </td>
        </tr>
    )
