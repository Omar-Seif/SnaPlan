...
const Step3ReviewSubmit: React.FC<Props> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Venue (existing or new)
      if (formData.venue.id) {
        ...
        formData.venue.rooms.forEach((room, i) => {
          data.append(`newVenue.Rooms[${i}].Name`, room.name);
          data.append(
            `newVenue.Rooms[${i}].Capacity`,
      ...
      sessions.forEach((s, i) => {
        data.append(`sessions[${i}].Name`, s.name);
        data.append(`sessions[${i}].Speaker.Name`, s.speaker.name);
        data.append(`sessions[${i}].Speaker.Bio`, s.speaker.bio);
        data.append(`sessions[${i}].Speaker.Profile`, s.speaker.profile);
        if (s.timeSlot) {
          data.append(`sessions[${i}].TimeSlot.Day`, s.timeSlot.day);
          data.append(
            `sessions[${i}].TimeSlot.StartDate`,
            s.timeSlot.startDate
          );
          data.append(`sessions[${i}].TimeSlot.EndDate`, s.timeSlot.endDate);
        }
      });
