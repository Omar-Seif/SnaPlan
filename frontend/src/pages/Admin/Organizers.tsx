...
const initialPendingOrganizers: PendingOrganizer[] = [
  {
    id: 101,
    name: "Mariam Nasser",
...
function Organizers() {
  ...
  const [pendingOrganizers, setPendingOrganizers] = useState(initialPendingOrganizers);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editForm, setEditForm] = useState<Partial<Organizer>>({});
  ...
  const fetchOrganizers = async () => {
    try {
      ...
      const transformedData: Organizer[] = data.map((org: any) => ({
        id: org.id,
  ...
  const deleteOrganizerAPI = async (id: number) => {
    try {
  ...
  const saveEdit = () => {
    ...
    setOrganizers((prev) =>
      prev.map((org) => (org.id === editingId ? { ...org, ...editForm } as Organizer : org))
    );
    setEditingId(null);
    setEditForm({});
  };
  ...
  const toggleBan = (id: number) => {
    setOrganizers((prev) =>
      prev.map((org) =>
        org.id === id
          ? { ...org, status: org.status === "inactive" ? "active" : "inactive" }
          : org
      )
    );
  };

  const approveOrganizer = (pendingOrg: PendingOrganizer) => {
    ...
    setPendingOrganizers((prev) => prev.filter((org) => org.id !== pendingOrg.id));
  };

  const rejectOrganizer = (id: number) => {
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2 text-center">
          Sna<span className="text-red-700 font-extrabold">Plan</span> Organizers Dashboard
        </h1>
      </div>
